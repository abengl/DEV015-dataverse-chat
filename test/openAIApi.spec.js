...
describe("communicateWithOpenAI", () => {
  ...
  it("should reject with an error if API key is not present", () => {
    ...
    return communicateWithOpenAI([]).catch((error) => {
  ...
  it("should handle 429 Too Many Requests response", () => {
    ...
    return communicateWithOpenAI([]).catch((error) => {
      expect(error).toEqual(
        new Error("Demasiadas solicitudes: Too Many Requests")
      );
    });
  });
  ...
  it("should handle 500 Internal Server Error response", () => {
    ...
    return communicateWithOpenAI([]).catch((error) => {
      expect(error).toEqual(
        new Error("Error interno del servidor: Internal Server Error")
      );
    });
  });
  ...
  it("should handle 502 Bad Gateway response", () => {
    ...
    return communicateWithOpenAI([]).catch((error) => {
      expect(error).toEqual(
        new Error("Puerta de enlace incorrecta: Bad Gateway")
      );
    });
  });
  ...
  it("should handle 503 Service Unavailable response", () => {
    ...
    return communicateWithOpenAI([]).catch((error) => {
      expect(error).toEqual(
        new Error("Servicio no disponible: Service Unavailable")
      );
    });
  });
  ...
  it("should handle 504 Gateway Timeout response", () => {
    ...
    return communicateWithOpenAI([]).catch((error) => {
      expect(error).toEqual(
        new Error("Tiempo de espera de la puerta de enlace: Gateway Timeout")
      );
    });
  });

  it("should handle any type of error response", () => {
    getApiKey.mockReturnValue("valid-api-key");
    global.fetch.mockResolvedValue({
      ok: false,
      status: 505,
      statusText: "HTTP Version Not Supported",
    }); // Simulate a 505 error response

    return communicateWithOpenAI([]).catch((error) => {
      expect(error).toEqual(
        new Error("Error en la petición a OpenAI: HTTP Version Not Supported")
      );
    });
  });
